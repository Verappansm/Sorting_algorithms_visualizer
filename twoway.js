document.addEventListener("DOMContentLoaded", function () {
    ...
    async function twoWaySort(arr) {
        ...
        while (i < j) {
            if (arr[i - 1] > arr[j - 1]) {
                ...
                await swapAndAnimate(children[i - 1], children[j - 1]);
            }
            i++;
            j--;
        }
        ...
        for (i = 1; i <= n / 2 && exc;) {
            ...
            for (let j = i; j <= n - i; j++) {
                if (arr[j - 1] > arr[j]) {
                    ...
                    await swapAndAnimate(children[j - 1], children[j]);
                }
                ...
                if (arr[n - j - 1] > arr[n - j]) {
                    ...
                    await swapAndAnimate(children[n - j - 1], children[n - j]);
                }
            }
            i++;
        }
    ...
    async function twoWaySorting() {
        if (sorted == false) {
            ...
            const sortingPromise = new Promise(async (resolve) => {
                await twoWaySort(arr);
                resolve();
            });
